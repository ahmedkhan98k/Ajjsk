...
CHANNEL_USERNAME2 = "@P_y_hy"  
DEVELOPER_IDS = [110484930]
...
def create_main_keyboard():
    keyboard = {
        'inline_keyboard': [
            ...
            [
                {'text': 'المطور', 'url': 'https://t.me/AHMED_KHANA'},
                {'text': 'قناة المطور', 'url': 'https://t.me/P_y_hy'}
            ],
            [
...
async def handle_start(user, chat_id):
    ...
    welcome_message = (
        "مرحبا بك بوت المطور احمد خان تحميل من جميع مواقع التواصل\n"
...
async def handle_main_button_click(callback_query, button_data):
    ...
    if button_data.startswith('download_'):
        ...
        welcome_message = (
            "مرحبا بك بوت تحميل من جميع مواقع التواصل\n"
            "تحميل بدون حقوق او علامة مائية\n"
            "اختر المنصة التي تريد التحميل منها:"
        )
        keyboard = create_main_keyboard()
...
async def handle_tiktok(chat_id, url, message_id=None):
    ...
    try:
        ...
        # محاولة ثانية باستخدام TikWM API
        headers = {
            'authority': 'www.tikwm.com',
            'accept': 'application/json, text/javascript, */*; q=0.01',
            'accept-language': 'ar,en;q=0.9',
        ...
        # محاولة ثالثة باستخدام SnapTik API
        headers_snaptik = {
        ...
        response = requests.post('https://snaptik.app/action-2024.php', headers=headers_snaptik, data=data_snaptik, timeout=20)
        ...
        if response.status_code == 200:
            ...
            for link in soup.find_all('a', href=True):
                href = link['href']
                if '.mp4' in href and ('download' in href or 'snaptik' in href):
                    download_links.append(href)
            
            if download_links:
                video_url = download_links[0]
                if not video_url.startswith('http'):
                    video_url = 'https://snaptik.app' + video_url
        ...
        if loading_msg_id:
            await delete_message(chat_id, loading_msg_id)
        await send_message(chat_id, "❌ عذراً، تعذر تحميل الفيديو من تيك توك\nحاول مرة أخرى أو استخدم رابط آخر")
        ...
        await send_message(chat_id, "❌ حدث خطأ في الاتصال، حاول مرة أخرى")
        ...
        await send_message(chat_id, "❌ حدث خطأ غير متوقع، حاول مرة أخرى")
...
async def handle_pinterest(chat_id, url, message_id=None):
    ...
    try:
        ...
        await send_message(chat_id, "❌ حدث خطأ في تحميل المحتوى")
...
async def handle_facebook(chat_id, url, message_id=None):
    ...
    try:
        ...
        await send_message(chat_id, "❌ حدث خطأ في تحميل الفيديو")
...
async def handle_message(user, chat_id, text, message_id=None):
    ...
    try:
        low = text.lower()
        if "tiktok.com" in low or "tiktok" in low:
        ...
        elif "facebook.com" in low or "fb.com" in low or "fb.watch" in low:
        ...
        elif "youtube.com" in low or "youtu.be" in low or "youtube" in low:
